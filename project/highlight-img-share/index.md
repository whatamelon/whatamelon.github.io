...
주요 플로를 정리했다. 크게 `편집 모드`와 `편집이 완료된 화면`으로 구성했다. 편집이 완료된 화면은 사실 없애도 될 단계이다. 편집 모드에서 공유를 누르면 바로 share extension을 띄워주면 되기 때문이다. 하지만 편집이 완료되었다는 느낌을 주고 싶었고 많이 공유되는 앱을 전면에 배치해서 share extension에서 앱을 찾아야 하는 불편을 줄이고 싶었다.
...
가운데 정렬보다는 글을 더 잘 읽을 수 있는 왼쪽 정렬을 사용하기로 결정했다. 제일 중요한 책의 구절이 먼저 보이도록 최상단에 배치했다. 문장 아래에는 어떤 책에서 발췌한 문장인지 알 수 있게 책 제목과 저자명을 추가했고, 리디북스로 책을 읽은 구절임을 알 수 있게 로고를 우측 하단에 배치했다.
